...
/* ───── password reset flow ───── */
async function handlePasswordResetRequest(req: any, res: any, next: any) {
  try {
    const { email } = resetRequestSchema.parse(req.body);

    const authClient = createSupabaseAuthClient();
    const { error } = await authClient.auth.resetPasswordForEmail(email, {
      redirectTo: `${FRONTEND_URL?.replace(/\/$/, "") ?? ""}/password-reset/confirm`,
    });
...
async function handlePasswordResetConfirm(req: any, res: any, next: any) {
  try {
    const { email, verificationCode, password } = resetConfirmSchema.parse(
      req.body,
    );
    ...
    if (error || !data?.user) {
      throw error ? mapSupabaseError(error) : new AppError(400, "Invalid or expired code");
    }
    ...
    if (updateError) {
      throw mapSupabaseError(updateError as AuthError);
    }
